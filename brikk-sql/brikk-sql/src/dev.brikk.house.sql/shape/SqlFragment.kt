    ...
    private fun buildSchema(inputs: ShapeCatalog, rejectNormalizedDuplicates: Boolean = false): MappingSchema {
        ...
        fun columns(shape: Shape): Map<String, String> = buildMap {
            ...
            for (column in shape.columns) {
                val quoted = column.quoted || !simpleName.matches(column.name)
                val identifier = Identifier(args("this" to column.name, "quoted" to quoted))
                val normalized = dialectObj.normalizeIdentifier(identifier.copy()).name
                ...
                put(if (quoted) dialectObj.generate(identifier) else column.name, column.type)
            }
        }

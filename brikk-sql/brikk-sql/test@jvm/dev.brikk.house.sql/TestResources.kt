 ...
 *
 * Resolution order: module classpath, then `brikk-sql/brikk-sql/testResources/` (CWD = repo root,
...
internal fun testResourceOrNull(path: String): String? {
    val stream = TestResources::class.java.classLoader.getResourceAsStream(path)
        ?: File("brikk-sql/brikk-sql/testResources/$path").takeIf { it.exists() }?.inputStream()

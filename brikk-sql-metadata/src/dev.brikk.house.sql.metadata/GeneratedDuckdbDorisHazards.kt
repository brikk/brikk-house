// GENERATED FILE — DO NOT EDIT.
// Generated by tools/generate_hazards_registry.py from
// brikk-sql/testResources/semantics/duckdb-doris-hazards.json (brikk live-probe program,
// extracted null): live-probe-verified duckdb<->doris semantics
// verdicts. Regenerate after editing the JSON; the run is byte-deterministic.
//
// Keying and collision policy: see the tool's docstring / HazardRegistry KDoc
// (raw side-name + bare-identifier alternatives; collisions keep the WORST
// verdict, ties keep JSON order).
package dev.brikk.house.sql.metadata

/** The 0 probe-verified (duckdb, doris) pair verdicts, in JSON order. */
internal val DUCKDB_DORIS_HAZARD_ENTRIES: List<FunctionHazard> = emptyList()

/** duckdb->doris lookup: 0 keys (DuckDB-side names) over 0 entries. */
internal val DUCKDB_TO_DORIS_HAZARDS: Map<String, FunctionHazard> = buildMap {
}

/** doris->duckdb lookup: 0 keys (Doris-side names) over 0 entries. */
internal val DORIS_TO_DUCKDB_HAZARDS: Map<String, FunctionHazard> = buildMap {
}

...
@BrikkSql
fun eventsInRange(start: Instant, end: Instant) = Sql.postgres("""
    FROM public.events
    |> WHERE event_at >= :start AND event_at < :end
""")

@BrikkSql
fun <T : HasPayload> extractEvent(src: Rel<T>) = Sql.postgres("""
...
@BrikkSql
fun loginDaily(src: Rel<LoginInput>) = Sql.postgres("""
    |> WHERE action = 'login'

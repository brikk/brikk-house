...
# Release-branch additions to the existing master snapshot, extracted by the same
# signature tool with --classes. Kept separate so neither source pin is misrepresented.
RELEASE_ADDITIONS_JSON = ROOT / "vendor" / "data" / "doris-4.1.4-additions.json"
RELEASE_ADDITIONS_SHA = "ad35a140c7fd0b842f18c23300bac581f7d04326"
...
def main() -> None:
    ...
    additions = json.loads(RELEASE_ADDITIONS_JSON.read_text(encoding="utf-8"))
    if additions["source_sha"] != RELEASE_ADDITIONS_SHA:
        sys.exit("error: unexpected Doris release supplement source pin")
    # A future full refresh may subsume these definitions. Keep that snapshot's
    # signatures when present; never create duplicate definitions or aliases.
    known_names = {n for entries in by_kind.values() for _, name, aliases in entries for n in [name, *aliases]}
    for cls, entry in additions["classes"].items():
        names = [n.upper() for n in entry["names"]]
        if any(n in known_names for n in names):
            continue
        by_kind[entry["kind"]].append((cls, names[0], names[1:]))
        signatures_by_class[cls] = entry["signatures"]
        if entry["nullable_mode"]:
            modes_by_class[cls] = entry["nullable_mode"]
    ...
    lines = [
        ...
        f"// Plus release-branch additions from 4.1.4 ({RELEASE_ADDITIONS_SHA}),",
        "// extracted in vendor/data/doris-4.1.4-additions.json. This is a combined catalog,",
        "// not an exact registry dump of a single Doris release.",

...
def load_pair_detail(gap: dict, source: str, target: str) -> dict:
    """Resolve an ordered pair's full detail record via the manifest index.

    The manifest maps each pair to a detail file under `function-gaps/`; that file
    holds the pair's metadata, counts and full `entries` list.
    """
    key = f"{source}->{target}"
    idx = gap["pairs"].get(key)
    if idx is None:
        sys.exit(f"error: gap-report.json manifest missing pair {key!r}")
    rel = idx.get("file")
    if not rel:
        sys.exit(f"error: gap-report.json manifest pair {key!r} has no 'file' pointer")
    detail_path = SEMANTICS / rel
    if not detail_path.exists():
        sys.exit(f"error: pair detail file missing: {detail_path.relative_to(ROOT)}")
    return json.loads(detail_path.read_text())


def collect_bucket_a(gap: dict, source: str, target: str) -> list[dict]:
    detail = load_pair_detail(gap, source, target)
    return [e for e in detail["entries"] if e.get("bucket") == "A"]
...
def main() -> None:
    ...
    header = [
        ...
        f"(+ its `function-gaps/` pair files) + `trino-duckdb-hazards.json`. "
        f"Re-run is byte-deterministic._",
        "",
    ]

...
Outputs (regenerated on every run, deterministic):
  - brikk-sql/brikk-sql/testResources/semantics/gap-report.json         (small manifest/index)
...
def write_markdown(pairs_data, pair_metadata, catalogs, evidence, path):
    ...
    w("the manifest `brikk-sql/brikk-sql/testResources/semantics/gap-report.json`, and the per-pair")
    w("detail files under `brikk-sql/brikk-sql/testResources/semantics/function-gaps/`).")
    w("")
    w("## Report layout")
    w("")
    w("The machine-readable report is split to keep reviews and merges manageable:")
    w("")
    w("- `brikk-sql/brikk-sql/testResources/semantics/gap-report.json` - small deterministic")
    ...
    w("- `brikk-sql/brikk-sql/testResources/semantics/function-gaps/<src>__<tgt>.json` - one file per")
...
def main():
    ...
    removed = write_split_report(
        ...
        ROOT / "brikk-sql" / "brikk-sql" / "testResources" / "semantics" / "gap-report.json",
    )

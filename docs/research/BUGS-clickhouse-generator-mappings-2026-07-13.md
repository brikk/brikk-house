...
duckdb `log->log10`, `xor->bitXor`, `week->toISOWeek`, and trino `week->toISOWeek`. The
duckdb `regexp_replace->replaceRegexpOne` fix was reconciled to `conditionally-equivalent`
(not identical): the common first-match case is now correct, but a residual empty-pattern
divergence stays surfaced. Registry regenerated byte-clean; counts unchanged
(duckdb↔clickhouse 152: now 48 divergent / 69 identical / 35 cond-eq; trino↔clickhouse
